...
# wrapper to change print model to invisible
...
, where the `A` and `I` nodes are the activating and inhibiting regulators respectively of the target node `T` and the `AND NOT` is the **link (balance) operator**, we can specify a `data.frame` object whose values (corresponding to a specific **model-node element**) can be one of the following:
...
Note that it's generally better to use the MCC classification, since it's a more robust performance evalution metric compared to the number of TP predictions, since it takes into account all of the four confusion matrix values.
...
- We can choose the number of model groups to be created (`num.of.mcc.classes`, with default value 5).
...
We can also plot the MCC-model histogram, which in addition shows the *density* (how many models) and *width* (MCC range) of each MCC class:
...
- All values are in the $[-1,1]$ interval. 
The more negative the activity difference value, the more inhibited is the node in the better performance models (e.g. `PTK2`). 
The more positive the activity difference value, the more active is the node in the better performance models (e.g. `PPM1A,PTEN`).
- Based on a **user-given** `threshold` level, a node is declared as an **activity biomarker** if it’s highest absolute value surpasses that threshold (see `emba::get_biomarkers()` for more info).
...
we conclude that **the only activity biomarker of interest** is `PTEN` as it's activity state directly influences the `PPM1A` and `PTK2` nodes.
...
mcc.analysis.res$diff.link.mcc.mat %>% 
  as.data.frame() %>%
  select(c("AKT","PTEN","PSEN1","CEBPA")) %>% # show only part of the matrix
  kable(caption = "Average Link Operator Difference Matrix")
```
...
The core idea behind the implementation is that the models are now classified to groups based on whether they **predict or not each of the predicted synergies** (which for the test dataset are the same 5 as found with the previous analyses).
...
Now in addition to the predicted synergies, we get all the **subsets** for which a model predicted all drug combinations in that subset as *synergistic*.
We can visualize this results with the `emba::make_barplot_on_synergy_subset_stats()` function:
...
Next, we can visualize the *activity state differences* result for each of the model group comparisons (per predicted synergy) with a heatmap:
```{r synergy-analysis-3, fig.align='center', fig.width=7, fig.height=5}
ht = ComplexHeatmap::Heatmap(matrix = synergy.analysis.res$diff.state.synergies.mat,
  name = "State Diff",
  col = circlize::colorRamp2(c(-1, 0, 1), c("tomato", "grey", "gold")),
  column_title = paste0("Average State Differences Heatmap"),
  column_title_gp = gpar(fontsize = 14, fontface = "bold"),
  row_title = "Predicted synergies", row_names_side = "left",
  row_order = 1:nrow(synergy.analysis.res$diff.state.synergies.mat),
...
The heatmap shows that there are patterns in the data - i.e. there are common nodes who affect the prediction of several synergies.

Of course, every row of that above matrix can be also network-plotted.
We show the average state difference graph for the `PI-D1` synergy:
...
We can visualize them in another heatmap:
...
ht = ComplexHeatmap::Heatmap(matrix = as.matrix(biomarker.act.mat),
  col = circlize::colorRamp2(c(-1, 0, 1), c("tomato", "grey", "gold")),
  column_title = paste0("Activity State Biomarkers"),
  column_title_gp = gpar(fontsize = 14, fontface = "bold"),
  row_title = "Predicted synergies", row_names_side = "left",
  row_order = 1:nrow(biomarker.act.mat),
...
As we can observe, with the given configuration, we couldn't find activity biomarkers for every predicted synergy.
This is justifiable, since the number models that predict a synergy vs the the number of models that don't predict it (i.e. predict *antagonism*) can be fairly unbalanced and the `penalty` term corrects for this.
...
pretty_print_string(paste0("Number of models (AK-PD): #Synergistic: ", syn.models.num, ", #Antagonistic: ", ant.models.num))
```

Lastly, we get the **average link operator differences** per network node (whose boolean equation had a link operator) when comparing the predicted synergy vs antagonism model groups in the `synergy.analysis.res$diff.link.synergies.mat` matrix result.
The filtered link operator biomarkers (based on the given `threshold`) is given in the `synergy.analysis.res$link.operator.biomarkers` output.
Both results can be visualized the same way as shown above.
